  ...
  selectAction(wf, board) {
    ...
    if (wf.step === 'main_phase') {
      ...
      if (wf.attackable && wf.attackable.length > 0) {
        ...
        const oppGigs     = oppState?.zones?.gigs || [];
        ...
        for (const attackerIid of wf.attackable) {
          ...
          let target = null;
          if (tgt.gigs && oppGigs.length > 0)         target = { kind: 'gigs' };
          else if ((tgt.unit_iids || []).length > 0)  target = { kind: 'unit', iid: tgt.unit_iids[0] };

          if (!target) {
            if (!compelled) continue;
            if ((tgt.unit_iids || []).length > 0)     target = { kind: 'unit', iid: tgt.unit_iids[0] };
            else if (tgt.gigs)                        target = { kind: 'gigs' };
            else continue;
          }
