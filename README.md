...
| `server-ai-mybot-v3.js` | The strongest shipped example — look-ahead bot. Plays each candidate move on a private copy of the game with the rules engine ([cyber-sim-engine](https://github.com/orzdk/cyber-sim-engine), cloned side-by-side or installed as a package) and scores the resulting board, so it handles every card without per-card code. Falls back to script-driven heuristics when the engine isn't found. Registered server-side as `punkbot-pro-light`. |
...
| `this.pickIid(cn)` | Candidate for an iid prompt: the first rival-controlled card for unit / legend / gear prompts that span both sides, else the first candidate; `null` when empty |
| `this.controlsIid(iid)` | True when that unit, legend or piece of gear in play is yours |
...
`kind` is `unit`, `legend` or `gear`. A gear has no state: the `{Spend}` cost of
a gear's ability is paid by spending the unit or legend it is equipped to
(`host_iid`), which must be ready. Activate it with the gear's `iid`; the web
client offers it on the host card.
...
{ step: 'play_legend_solo', iid }                // Go Solo: face-up legend (ready or spent) → ready unit that can attack now. Solo cost (rival taxes/discounts apply).
{ step: 'play_legend',    iid }                  // plain play of a Go Solo legend: printed cost, keeps its ready/spent state, no attack this turn
...
For a prompt whose candidates may sit on either side (`side: 'both'` in the
card script — "defeat a Gear", "a Unit can't attack"), the engine lists p1's
cards before p2's, so the first candidate is not always the one you want. The
bundled bots answer unit / legend / gear prompts with `this.pickIid(cn)`, which
takes the first candidate the rival controls.
...
```js
// choice_needed: { kind: 'choose_units', zone, prompt, available_iids, take_up_to, optional, bind_pid, chooser_pid }
// zone is 'field' (units) or 'legends' (e.g. "ready up to 2 Merc Legends"); the answer shape is the same
{ step: 'effect_choice_response', response: { selected_iids: [iid1, iid2] } }
```
...
- **Silent room loss triggers a re-warm; explicit eviction doesn't.** If the SSE stream drops and reconnect finds the room gone (404), a *hosting* bot creates a new room with the same deck and carries on — seat-mode bots shut down instead (their seat was server-issued). An explicit `evicted` SSE event is a server order: the base class shuts down cleanly. The same goes for `closed` (the server ended the game, e.g. for a system update — bot games are never carried across one). If the host stays unreachable after a few reconnect attempts, the bot stops with `host_unreachable`.
...
GET  /api/rooms/:id/events?token=<token>                                    -> SSE: state, log, alert, closed, evicted,
                                                                                    maintenance, rematch-suggested, ping (heartbeat)
